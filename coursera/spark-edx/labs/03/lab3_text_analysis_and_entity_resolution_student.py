...
def simpleTokenize(string):
    ...
    """
    new_text = re.split(split_regex, string.lower().strip())
    return filter(None, new_text)
...
def countTokens(vendorRDD):
    ...
    """
    textRDD = vendorRDD.map(lambda (x, y): y)
    textRDD = textRDD.flatMap(lambda text: tokenize(text))
    
    return textRDD.count()
...
def findBiggestRecord(vendorRDD):
    ...
    recordTokenPair = vendorRDD.map(lambda (x, y): (x, tokenize(y)))
    return recordTokenPair.takeOrdered(1, lambda s: -1 * len(s[1]))
