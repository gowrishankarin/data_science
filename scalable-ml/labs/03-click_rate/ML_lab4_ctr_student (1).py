...
aSparse = SparseVector(len(aDense), [1, 3], [3.0, 4.0])

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(len(bDense), [3], [1.0])
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, sorted([OHEDict[feat] for feat in rawFeats], key = int), [1 for feat in rawFeats])
